...
function questionKeyboard(step) {
  ...
  const shortLabelsByStep = {
    1: ['Хочется больше стройности', 'Хочу подтянуть зоны', 'Могу выглядеть лучше', 'Устала стесняться'],
    2: ['Немного «наливается» к вечеру', 'К вечеру ощущается тяжелее', 'Устаёт спина/поясница', 'Хочется больше тонуса'],
    3: ['Да, расстраивает', 'Иногда замечаю', 'Раньше было проще', 'Можно лучше', 'Перестала пытаться'],
...
bot.action('final_think', async (ctx) => {
  ...
  await safeReply(ctx, 'Окей 🙂 Здесь рядом, если захочешь вернуться.', undefined, 'final-think');
});
